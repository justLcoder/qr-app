...
function App() {
  ...
  function completeAuthentication(newToken: string) { localStorage.setItem("qr_token", newToken); setToken(newToken); navigate("/dashboard"); }
  ...
  return <main>
    ...
    {route === "/dashboard" ? <Dashboard token={token} t={t} onAuthenticated={completeAuthentication} onNavigate={navigate} onLogout={logout} /> : isAuthPage ? <AuthPage mode={route === "/login" ? "login" : "register"} t={t} onAuthenticated={completeAuthentication} onNavigate={navigate} /> : <Home token={token} t={t} onNavigate={navigate} />}
  </main>;
}
...
function Header({ token, language, theme, t, authPage, onNavigate, onLanguage, onTheme, onLogout }: { token: string; language: Language; theme: Theme; t: Copy; authPage: boolean; onNavigate: (path: string) => void; onLanguage: (language: Language) => void; onTheme: () => void; onLogout: () => void }) {
  return <header><button className="brand" onClick={() => onNavigate("/")}>QR Studio</button><span className="tagline">{t.tagline}</span><div className="header-actions"><label className="language-picker"><GlobeIcon /><span className="sr-only">Language</span><select value={language} onChange={event => onLanguage(event.target.value as Language)}>{languages.map(item => <option value={item.code} key={item.code}>{item.label}</option>)}</select></label><button className="icon-button" onClick={onTheme} aria-label="Toggle color theme"><ThemeIcon theme={theme} /></button>{authPage ? <button className="quiet" onClick={() => onNavigate("/")}>{t.backToGenerator}</button> : token ? <><button className="quiet" onClick={() => onNavigate("/dashboard")}>{t.dashboard}</button><button className="text-button" onClick={onLogout}>{t.signOut}</button></> : <button className="quiet" onClick={() => onNavigate("/login")}>{t.signIn}</button>}</div></header>;
}
...
function Home({ token, t, onNavigate }: { token: string; t: Copy; onNavigate: (path: string) => void }) {
  ...
  const [preview, setPreview] = useState(""); const [created, setCreated] = useState<Code | null>(null); const [notice, setNotice] = useState("");
...
function Dashboard({ token, t, onAuthenticated, onNavigate, onLogout }: { token: string; t: Copy; onAuthenticated: (token: string) => void; onNavigate: (path: string) => void; onLogout: () => void }) {
  ...
  async function remove(code: Code) { if (!confirm(`${t.delete} ${code.label || t.untitled}?`)) return; try { await request(`/api/qr-codes/${code.id}`, { method: "DELETE" }, token); loadCodes(); } catch (error) { setNotice(error instanceof Error ? error.message : "Delete failed"); } }
  return <><section className="dashboard-page"><div className="dash-title"><p className="eyebrow">{t.workspaceEyebrow}</p><h1>{t.savedCodes}</h1><p className="lede">{t.authDescription}</p></div>{notice && <p className="notice dashboard-notice">{notice}</p>}{loading ? <p className="empty">Loading…</p> : codes.length ? <div className="code-list">{codes.map(code => <article key={code.id}><div><strong>{code.label || t.untitled}</strong><p>{code.destination_url}</p></div><b>{code.scan_count} {t.scans}</b><button onClick={() => editDestination(code)}>{t.editLink}</button><button className="danger" onClick={() => remove(code)}>{t.delete}</button></article>)}</div> : <p className="empty">{t.noCodes}</p>}</section><footer>QR Studio · {t.footer}</footer></>;
}
